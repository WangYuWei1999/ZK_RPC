#ifndef REST_RPC_CODEC_H
#define REST_RPC_CODEC_H
...
using buffer_type = msgpack::sbuffer;    //buffer缓冲区
struct msgpack_codec{
    const static size_t init_size = 2*1024;

    template<typename... Args> 
    static buffer_type pack_args(Args&& ...args){
        buffer_type buffer(init_size);      //分配空间
        //std::forward_as_tuple(std::forward<Args>(args)...);接受右值引用数据构建tuple
