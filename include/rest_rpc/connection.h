...
//enable_shared_from_this 在类对象内部获取一个指向当前对象的shared_ptr对象
...
public:
    connection(asio::io_service& io_service, std::size_t timeout_seconds, router& router)
        :socket_(io_service), body_(INIT_BUF_SIZE), timer_(io_service),timeout_seconds_(timeout_seconds),has_closed_(false), router_(router){}
    ...
    //返回远程地址
    std::string remote_adress() const {
        if(has_closed_) return"";

        asio::error_code ec;                 
        //remote_endpoint 获取通信的客户端的ip和端口                            
...
private:
    void read_head(){
        reset_timer();
        auto self(this->shared_from_this()); //类内部返回指向当前对象的shared_ptr
        async_read_head([this, self](asio::error_code ec,std::size_t length){
            ...
            if(!ec){
                rpc_header* header = (rpc_header*)(head_);     //异步读取到head_中
                if(header->magic != MAGIC_NUM){
                    print("protocol error");
                    close();
                    return;
                }
                req_id_ = header->req_id; //获取请求id
                const uint32_t body_len = header->body_len;  //获取请求体长度
                req_type_ = header->req_type;  //获取请求类型 一般请求/订阅发布者模式
                
                if(body_len > 0&& body_len < MAX_BUF_LEN)   {
                    if(body_.size() < body_len){
                        body_.resize(body_len);
                    }
                    read_body(header->func_id, body_len); //函数名哈希值，请求体长度
                    return;
                }
                if(body_len == 0){
                    cancel_timer();
...
//读取正文部分
    void read_body(uint32_t func_id, std::size_t size){
        ...
        async_read(size,[this, func_id, self](asio::error_code ec, std::size_t length){
            ...
            if(!ec){
                read_head();  //继续异步循环读取，以下解析本次读取的请求
                if(req_type_ == request_type::req_res){
                    //解析报文并作出响应
                    router_.route<connection>(func_id, body_.data(),length,this->shared_from_this()); //真正调用本地方法在router部分
                }else if(req_type_ == request_type::sub_pub){
                    try{
                        msgpack_codec codec;  //反序列化正文内容
                        auto p = codec.unpack<std::tuple<std::string, std::string>>(body_.data(),length);
                        callback_(std::move(std::get<0>(p)), std::move(std::get<1>(p)),this->shared_from_this());     //发布者模式回调
                    }catch(const std::exception& ex){
                        print(ex);
                        if(on_net_err_) (*on_net_err_)(self,ex.what());
                    }
                }
            }else{
                if(on_net_err_) (*on_net_err_)(self,ec.message());
            }
        });
    }
    ...
    void write(){
        ...
        header_ = {MAGIC_NUM,  msg.req_type, write_size_, msg.req_id};   //构建rpc_header
        std::array<asio::const_buffer, 2> write_buffers;                //缓冲区数组
        write_buffers[0] = asio::buffer(&header_, sizeof(rpc_header));  //头部
        write_buffers[1] = asio::buffer(msg.content->data(),write_size_); //响应内容
    ...
    void on_write(asio::error_code ec, std::size_t length){
        if(ec){
            print(ec);
            if(on_net_err_) (*on_net_err_)(shared_from_this(),ec.message());
            close(false);
            return;
        }
    ...
    void async_handshake(){
        ...
        ssl_stream_->async_handshake(asio::ssl::stream_base::server,[this,self](const asio::error_code& error){
            if(error){
                print(error);
                if(on_net_err_) (*on_net_err_)(self,error.message());
                close();
                return;
            }
            has_shake_ = true;
...
//重置定时器
    void reset_timer(){
        ...
        auto self(this->shared_from_this());
        timer_.expires_from_now(std::chrono::seconds(timeout_seconds_)); //设置过期时间
        timer_.async_wait([this,self](const asio::error_code& ec){
            if(has_closed()) return;
            if(ec) return;
            close(false);  //过期时间到了关闭当前连接
        });
    }
    ...
    std::function<void(std::string, std::string, std::weak_ptr<connection>)> callback_;  //发布者模式回调
